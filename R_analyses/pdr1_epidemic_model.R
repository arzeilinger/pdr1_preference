...
phi_pcBar <- rateParams %>% dplyr::filter(., parameter == "p1" & week.trt == "8S") %>% dplyr::select(., estimate, se)*24
phi_pcVec <- simulateData(phi_pcBar$estimate, phi_pcBar$se)

phi_piBar <- rateParams %>% dplyr::filter(., parameter == "p1" & week.trt == "12S") %>% dplyr::select(., estimate, se)*24
phi_piVec <- simulateData(phi_piBar$estimate, phi_piBar$se)

phi_mucBar <- rateParams %>% dplyr::filter(., parameter == "mu1" & week.trt == "8S") %>% dplyr::select(., estimate, se)*24
phi_mucVec <- simulateData(phi_mucBar$estimate, phi_mucBar$se)

phi_muiBar <- rateParams %>% dplyr::filter(., parameter == "mu1" & week.trt == "12S") %>% dplyr::select(., estimate, se)*24
...
beta_cBar <- transSummary %>% dplyr::filter(., week == "8" & trt == "S") %>% dplyr::select(., propInfected, sepropI)
beta_cVec <- simulateData(mean = beta_cBar$propInfected, SE = beta_cBar$sepropI) 

beta_iBar <- transSummary %>% dplyr::filter(., week == "12" & trt == "S") %>% dplyr::select(., propInfected, sepropI)
...
prop.i <- as.numeric(transSummary[transSummary$week == 3 & transSummary$trt == "S", "sourcepInfected"])
prop.f <- as.numeric(transSummary[transSummary$week == 12 & transSummary$trt == "S", "sourcepInfected"])
...
# Duration originall given in minutes, convert to fraction of a day
...
parS <- cbind(alpha_cVec, alpha_iVec,
              beta_cVec, beta_iVec,
...
ti <- proc.time()
simS <- apply(parS, 1, SECIMSimulations) %>% rbindlist() %>% as.data.frame()
tf <- proc.time()
...
summaryS <- data.frame("mean" = apply(simS, 2, median),
                       "sd" = apply(simS, 2, sd),
                       "cil" = apply(simS, 2, function(x) quantile(x, 0.025)),
                       "ciu" = apply(simS, 2, function(x) quantile(x, 0.975)),
                       "max" = apply(simS, 2, max))
summaryS
